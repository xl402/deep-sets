...
class NearestNeighbourGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        seq_len = np.random.randint(2, self.max_len + 1)
        x_data, y_data = [], []
    ...
    def __repr__(self):
        params = 'dim={}, max_len={}, data_size={}, batch_size={}'
        params = params.format(self.dim, self.max_len, self.data_size, self.batch_size)
        out = 'NearestNeighbourGenerator({})'
        return out.format(params)

    @staticmethod
    def _get_nearest_neighbours(sequence_len, dim):
        xs = np.random.randn(sequence_len, dim)
